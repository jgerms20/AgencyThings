...
function clearWorkspace() {
  ...
  reviewList.innerHTML = 'Upload or paste a plan and click <strong>Run workflow</strong> to see placement matches.';
  ...
  sourceList.textContent = 'Image search links, source links, and verification criteria will appear here.';
  resetWorkflowState();
}
...
function parsePlan(input) {
  ...
  const headerIndex = rows.findIndex((cells) => looksLikeHeader(cells));
  if (headerIndex >= 0) {
    const headers = rows[headerIndex].map(normalizeHeader);
    const dataRows = rows.slice(headerIndex + 1).filter((cells) => cells.some(Boolean));
    const indexes = {
      platform: findHeaderIndex(headers, ['platform', 'publisher', 'channel', 'site', 'partner', 'network']),
      placement: findHeaderIndex(headers, ['placement', 'ad format', 'format', 'tactic', 'deliverable', 'unit', 'inventory']),
      size: findHeaderIndex(headers, ['size', 'dimension', 'dimensions', 'ratio', 'spec', 'specs', 'asset size']),
      units: findHeaderIndex(headers, ['units', 'qty', 'quantity', 'count', 'number']),
      notes: findHeaderIndex(headers, ['notes', 'description', 'details', 'comments', 'creative notes'])
    };

    return dataRows.map((cells) => rowFromHeaderMap(cells, indexes));
  }
...
function rowFromHeaderMap(cells, indexes) {
  const fallbackText = cells.join(' ');
  return {
    platform: cellAt(cells, indexes.platform),
    placement: cellAt(cells, indexes.placement) || fallbackText,
    size: cellAt(cells, indexes.size),
    units: cellAt(cells, indexes.units),
    notes: [cellAt(cells, indexes.notes), indexes.notes >= 0 ? '' : fallbackText].filter(Boolean).join(' · ')
  };
}
...
function findBestPlacement(row) {
  ...
  for (const placement of placementLibrary) {
    const candidates = [placement.platform, placement.placement, ...placement.aliases].map((candidate) => ({
      original: candidate,
      normalized: normalize(candidate)
    }));
    const scoredCandidates = candidates.map((candidate) => {
      if (!candidate.normalized) return { ...candidate, score: 0, tokens: [] };
      const candidateTokens = candidate.normalized.split(' ').filter((token) => token.length > 1);
      const matchingTokens = candidateTokens.filter((token) => searchText.includes(token));
      return {
        ...candidate,
        score: searchText.includes(candidate.normalized) ? 1 : matchingTokens.length / Math.max(candidateTokens.length, 1),
        tokens: matchingTokens
      };
    });
    const strongest = scoredCandidates.reduce((highest, candidate) => candidate.score > highest.score ? candidate : highest, scoredCandidates[0]);

    if (strongest.score > best.confidence) {
      best = {
        placement,
        confidence: strongest.score,
        searchText,
        signals: scoredCandidates
...
function renderReview(items) {
  ...
  if (!items.length) {
    reviewList.className = 'review-list empty-state';
    reviewList.textContent = 'No usable placement rows found. Upload a file or paste a deliverables list and try again.';
    return;
  }
  ...
  for (const item of items) {
    ...
    card.title = isMatched
      ? 'This match is based on platform/placement aliases and token overlap. Confirm it is the exact paid placement before exporting.'
      : 'No confident placement match was found. Edit the row or add this placement to the spec library.';
    const confidence = Math.round(item.confidence * 100);
    card.innerHTML = isMatched
      ? `<strong>${escapeHtml(item.raw.platform || item.matchedPlacement.platform)} · ${escapeHtml(item.raw.placement || item.matchedPlacement.placement)}</strong><p>Matched to <b>${escapeHtml(item.matchedPlacement.platform)} ${escapeHtml(item.matchedPlacement.placement)}</b> · <span class="confidence">${confidence}% confidence</span></p>${buildConfidenceDetails(item)}<p class="match-explain">Hover note: verify this is the exact placement, objective, ratio, and copy field set.</p>`
      : `<strong>${escapeHtml(item.raw.platform || 'Unknown platform')} · ${escapeHtml(item.raw.placement || 'Unknown placement')}</strong><p>No confident match yet. Add this placement to <code>data/placements.json</code> or edit the pasted row.</p><p class="match-explain">Hover note: this needs manual setup before the output is reliable.</p>`;
    reviewList.append(card);
  }
}

function buildConfidenceDetails(item) {
  const sourceLinks = item.matchedPlacement.sourceUrls
    .map((url) => `<a href="${escapeAttribute(url)}" target="_blank" rel="noreferrer">Verify on ${escapeHtml(new URL(url).hostname)}</a>`)
    .join('');
  const signals = item.matchSignals.length
    ? item.matchSignals.map((signal) => `<li><b>${escapeHtml(signal.original)}</b> matched ${Math.round(signal.score * 100)}%${signal.tokens.length ? ` via tokens: ${escapeHtml(signal.tokens.join(', '))}` : ''}</li>`).join('')
    : '<li>No detailed signals available.</li>';
  return `
    <details class="confidence-details">
      <summary>View confidence details + source links</summary>
      <p><strong>Imported row text:</strong> ${escapeHtml(item.searchText)}</p>
      <ul>${signals}</ul>
  ...
  for (const item of items) {
    ...
    if (!item.matchedPlacement) {
      wrapper.innerHTML = `
        <div>
          ...
          <p>No image-search package yet. Add a placement spec to unlock source links and example-search prompts.</p>
        </div>
    ...
    const clips = placement.exampleSearches
      .map((query) => {
        ...
        return `<div class="clip-tile"><span>Image search to review</span><strong>${escapeHtml(query)}</strong><a href="${escapeAttribute(searchUrl)}" target="_blank" rel="noreferrer">Open image search</a></div>`;
      })
      .join('');
    ...
    wrapper.innerHTML = `
      <div class="source-card-header">
        <div>
          ...
          <p>Open the searches, pick examples that match the placement, then verify against the source links.</p>
        </div>
        <span class="confidence-chip">${confidence}% match</span>
      </div>
      <div class="source-links">${links}</div>
      <ul class="verification-list"><li>Confirm the example is the same platform and placement.</li><li>Confirm visible dimensions/safe zones align with the spec source.</li><li>Clip only examples that show useful creative behavior, not just pretty visuals.</li></ul>
