...
export const cultureShaperIdAliases: Record<string, string> = {
  minecraft: "minecraft-franchise",
  pokemon: "pokemon-franchise",
};

export function resolveCultureShaperId(id: string): string {
  return cultureShaperIdAliases[id] ?? id;
}

export function getCultureShaperPublicIds(shaper: CultureShaper): string[] {
  const aliases = Object.entries(cultureShaperIdAliases)
    .filter(([, canonical]) => canonical === shaper.id)
    .map(([alias]) => alias);
  return [shaper.id, ...aliases];
}

function additional(seed: AdditionalSeed): CultureShaper {
  const id = resolveCultureShaperId(seed.id);
  const relatedEntities = seed.relatedEntities.map((entity) => {
    if (entity.kind !== "culture-shaper" || !cultureShaperIdAliases[entity.id]) return entity;
    const relatedId = cultureShaperIdAliases[entity.id];
...
const portraitOverrides: Record<string, string> = {
  ...
  "minecraft-franchise": "/culture/minecraft.jpg",
  "paw-patrol": "/culture/paw-patrol.png",
};
...
export function getCultureShaper(id: string): CultureShaper | undefined {
  const resolved = resolveCultureShaperId(id);
  return cultureShapers.find((shaper) => shaper.id === resolved);
}
