...
export function dedupeStories<T extends { sourceUrl: string }>(stories: T[]): T[] {
  const seen = new Set<string>();
  return stories.filter((story) => {
    const key = canonicalUrl(story.sourceUrl);
